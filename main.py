...
class ProceduralSequencerApp:
    ...
    def _setup_ui(self):
        ...
        # ==========================================
        # TOP LEFT OVERLAY: Instrument Selector
        # ==========================================
        self.overlay_frame = tk.Frame(self.canvas, bg="#2a2a35", bd=1, relief=tk.RAISED, padx=5, pady=5)

        tk.Label(self.overlay_frame, text="Current Tool:", fg=TEXT_COLOR, bg="#2a2a35",
                 font=("Arial", 10, "bold")).pack(side=tk.LEFT, padx=(5, 10))

        self.current_tool_var = tk.StringVar(value=self.default_instrument)
        self.tool_dropdown = tk.OptionMenu(self.overlay_frame, self.current_tool_var, *INSTRUMENTS,
                                           command=self.change_tool_from_dropdown)
        self.tool_dropdown.config(bg="#444", fg="white", highlightthickness=0, font=("Arial", 9))
        self.tool_dropdown.pack(side=tk.LEFT, padx=5)

        self.canvas.create_window(20, 20, window=self.overlay_frame, anchor="nw")
        ...
        # Bottom Fixed Frame for Save/Load
        bottom_frame = tk.Frame(self.panel, bg=PANEL_COLOR)
        bottom_frame.pack(side=tk.BOTTOM, fill=tk.X, pady=15, padx=15)
        tk.Button(bottom_frame, text="Save Project", bg="#2a5a8a", fg="white", relief=tk.FLAT,
                  command=self.save_project).pack(fill=tk.X, pady=5)
        tk.Button(bottom_frame, text="Load Project", bg="#444", fg="white", relief=tk.FLAT,
                  command=self.load_project).pack(fill=tk.X)

        # Scrollable Area for Settings
        self.scroll_canvas = tk.Canvas(self.panel, bg=PANEL_COLOR, highlightthickness=0)
        self.scroll_scrollbar = tk.Scrollbar(self.panel, orient="vertical", command=self.scroll_canvas.yview)
        self.scroll_frame = tk.Frame(self.scroll_canvas, bg=PANEL_COLOR)
        ...
        self.scroll_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)

        tk.Label(self.scroll_frame, text="CONTROL PANEL", bg=PANEL_COLOR, fg=ACCENT_COLOR, font=("Arial", 12, "bold"),
                 pady=10).pack(fill=tk.X)

        # --- CATEGORY: Global Settings ---
        global_pane = CollapsiblePane(self.scroll_frame, "Global & Playback", True)
        global_pane.pack(fill=tk.X, padx=5, pady=2)

        tk.Label(global_pane.content_frame, text="GLOBAL BPM", fg=ACCENT_COLOR, bg=PANEL_COLOR, font=("Arial", 9)).pack(
            anchor="w")
        tk.Scale(global_pane.content_frame, variable=self.global_bpm, from_=60.0, to=300.0, resolution=1.0,
                 orient=tk.HORIZONTAL, bg=PANEL_COLOR, fg="white", highlightthickness=0, bd=0).pack(fill=tk.X)
        tk.Button(global_pane.content_frame, text="Play All (Standard)", font=("Arial", 10, "bold"), bg="#3a3a45",
                  fg=TEXT_COLOR,
                  command=self.trigger_green_flag, relief=tk.FLAT, pady=5).pack(fill=tk.X, pady=(10, 5))

        # --- CATEGORY: Tools & Blueprints ---
        tools_pane = CollapsiblePane(self.scroll_frame, "Tools & Blueprints", True)
        tools_pane.pack(fill=tk.X, padx=5, pady=2)

        tk.Button(tools_pane.content_frame, text="Schematics & Blueprints", bg="#6a4c93", fg="white",
                  font=("Arial", 9, "bold"),
                  relief=tk.FLAT, pady=5, command=self.open_schematic_window).pack(fill=tk.X, pady=(0, 10))

        group_frame = tk.Frame(tools_pane.content_frame, bg=PANEL_COLOR)
        group_frame.pack(fill=tk.X, pady=(0, 10))
        tk.Button(group_frame, text="Group (Ctrl+G)", bg="#445", fg="white", font=("Arial", 8), relief=tk.FLAT,
                  command=self.create_group).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(0, 2))
        tk.Button(group_frame, text="Ungroup (Ctrl+U)", bg="#544", fg="white", font=("Arial", 8), relief=tk.FLAT,
                  command=self.ungroup).pack(side=tk.RIGHT, expand=True, fill=tk.X, padx=(2, 0))

        tk.Checkbutton(tools_pane.content_frame, text="Advanced Mode (Sharps #)", variable=self.advanced_mode,
                       bg=PANEL_COLOR,
                       fg="#aaa", selectcolor="#222", activebackground=PANEL_COLOR, activeforeground="#fff",
                       command=self.refresh_note_menu).pack(anchor="w")

        # --- NODE PROPERTIES (Hidden when nothing selected) ---
        ...
        self.empty_label = tk.Label(self.scroll_frame, text="\nSelect a node to view properties", fg="#777",
                                    bg=PANEL_COLOR, font=("Arial", 10, "italic"))
        self.empty_label.pack(pady=20)

        self.node_props_parent = tk.Frame(self.scroll_frame, bg=PANEL_COLOR)
        ...
        self.note_dropdown = tk.OptionMenu(basic_pane.content_frame, self.ui_vars["note"], *self.get_current_scale(),
                                           command=self.apply_settings)
        self.note_dropdown.config(bg="#333", fg="white", highlightthickness=0)
        ...
        inst_menu = tk.OptionMenu(basic_pane.content_frame, self.ui_vars["instrument"], *INSTRUMENTS,
                                  command=self.apply_settings)
        inst_menu.config(bg="#333", fg="white", highlightthickness=0)
        ...
        sliders1 = [("Length (Beats)", "length", 0.05, 4.0), ("Volume", "volume", 0.0, 1.0),
        ...
        mode_menu = tk.OptionMenu(routing_pane.content_frame, self.ui_vars["mode"], "Poly", "Splitter",
                                  command=self.apply_settings)
        mode_menu.config(bg="#333", fg="white", highlightthickness=0)
    ...
    def change_tool_from_dropdown(self, selected_tool):
        self.default_instrument = selected_tool

    def _on_panel_scroll(self, event):
        # Allow scrolling with mousewheel
        delta = -1 * (event.delta / 120) if event.delta else (1 if event.num == 5 else -1)
        self.scroll_canvas.yview_scroll(int(delta), "units")
    ...
    def _bind_events(self):
        ...
        self.scroll_canvas.bind("<MouseWheel>", self._on_panel_scroll)
        self.scroll_canvas.bind("<Button-4>", self._on_panel_scroll)
        self.scroll_canvas.bind("<Button-5>", self._on_panel_scroll)
        self.scroll_frame.bind("<MouseWheel>", self._on_panel_scroll)
    ...
    def refresh_inventory_ui(self):
        if not hasattr(self, 'inv_buttons_frame') or not self.inv_buttons_frame.winfo_exists():
            return
        for widget in self.inv_buttons_frame.winfo_children(): widget.destroy()
    ...
    def place_schematic(self, root_wx, root_wy):
        if not self.active_schematic: return
        ...
        for i, data in enumerate(schema):
            if "children" in data:
                for child_idx in data["children"]:
                    if child_idx in id_map: self.nodes[id_map[i]]["children"].append(id_map[child_idx])
    ...
    def set_default_instrument(self, index):
        if 0 <= index < len(INSTRUMENTS):
            ...
            self.current_tool_var.set(self.default_instrument)
    ...
    def create_node(self, wx, wy, parent_id=None, override_data=None):
        ...
        if not override_data and parent_id is not None and parent_id in self.nodes:
            ...
            if middle_notes: data["note"] = random.choice(middle_notes)

        if override_data: data.update(override_data)
        self.nodes[nid] = data
        return nid
    ...
    def refresh_note_menu(self):
        ...
        menu.delete(0, "end")
        for note in scale: menu.add_command(label=note, command=lambda n=note: self.select_note_from_menu(n))
        ...
        if path:
            try:
                ...
                for k, v in nodes_data.items():
                    ...
                    if v["note"] not in NOTE_FREQS: v["note"] = "C4"
                    self.nodes[int(k)] = v
    ...
    def get_node_at(self, wx, wy, radius=25):
        for nid, d in self.nodes.items():
            if (d["x"] - wx) ** 2 + (d["y"] - wy) ** 2 <= radius ** 2: return nid
        return None
        ...
        for pid, d in self.nodes.items():
            for cid in d["children"]:
                if cid in self.nodes:
                    cd = self.nodes[cid]
                    if dist_to_segment(wx, wy, d["x"], d["y"], cd["x"], cd["y"]) < threshold / self.zoom: return (pid,
                                                                                                                  cid)
        return None
    ...
    def on_left_down(self, event):
        ...
        if node_id is not None:
            if node_id not in self.selected_nodes: self.selected_nodes = {node_id}
    ...
    def on_left_drag(self, event):
        if self.active_schematic: return
        self.mouse_w = self.s2w(event.x, event.y)
        dist = math.hypot(self.mouse_w[0] - self.mouse_down_w[0], self.mouse_w[1] - self.mouse_down_w[1])
        if self.drag_mode == "maybe_spawn" and dist > 10 / self.zoom: self.drag_mode = "box"
    ...
    def on_left_up(self, event):
        ...
        if self.drag_mode == "maybe_spawn" and dist <= 10 / self.zoom:
            ...
            if target_id is None: target_id = self.create_node(wx, wy, parent_id=self.drag_start_id)
    ...
    def delete_logic(self, target_node=None, target_wire=None):
        if target_node and target_node in self.nodes:
            ...
            if target_node in self.selected_nodes: self.selected_nodes.remove(target_node)
            for d in self.nodes.values():
                if target_node in d["children"]: d["children"].remove(target_node)
        elif target_wire:
            pid, cid = target_wire
            if pid in self.nodes and cid in self.nodes[pid]["children"]: self.nodes[pid]["children"].remove(cid)
            if self.selected_wire == target_wire: self.selected_wire = None
        self.update_settings_ui()

    def on_right_click(self, event):
        if self.active_schematic: return self.set_active_schematic(None)
        ...
        if nid:
            ...
            wire = self.get_wire_at(wx, wy)
            if wire: self.delete_logic(target_wire=wire)

    def on_delete_key(self, event):
        for nid in list(self.selected_nodes): self.delete_logic(target_node=nid)
        if self.selected_wire: self.delete_logic(target_wire=self.selected_wire)
    ...
    def paste_selection(self, event=None):
        ...
        for item in self.clipboard:
            for old_child in item["children"]: self.nodes[id_map[item["old_id"]]]["children"].append(id_map[old_child])
        self.selected_nodes = new_selection
        self.update_settings_ui()

    def on_spacebar(self, event=None):
        self.scheduler.clear()
        for nid in self.selected_nodes: self.trigger_node(nid)

    def trigger_green_flag(self, event=None):
        self.scheduler.clear()
        for nid, d in self.nodes.items():
            if d.get("start_on_flag", False): self.trigger_node(nid)
    ...
    def render_loop(self):
        for nid, d in self.nodes.items():
            ...
            if nid in self.selected_nodes: target_scale = 1.1
            d["anim_scale"] += (target_scale - d["anim_scale"]) * 0.2
            if d["flash"] > 0.01: d["flash"] += (0 - d["flash"]) * 0.1
        ...
        for nid, d in self.nodes.items():
            ...
            # Look up style from config, defaulting to circle if missing
            inst = d["instrument"]
